 ...
 */
function registerFindReferencesCommand(_context: vscode.ExtensionContext, _client: LanguageClient): vscode.Disposable {
  return vscode.commands.registerCommand('protobuf.findReferences', async (location?: ReferenceLocation) => {
    // Use location info from the CodeLens when provided; otherwise fall back to the active editor.
    const targetUri = location?.uri ? vscode.Uri.parse(location.uri) : vscode.window.activeTextEditor?.document.uri;
