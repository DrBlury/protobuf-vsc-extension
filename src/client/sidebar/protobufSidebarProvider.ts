...
export class ProtobufSidebarProvider implements vscode.TreeDataProvider<TreeItemType> {
    ...
    private async scanWorkspace(): Promise<void> {
        ...
        // Count services
        let services = 0;
        for (const file of protoFiles.slice(0, 50)) { // Limit for performance
            try {
                const doc = await vscode.workspace.openTextDocument(file);
                const text = doc.getText();
                const matches = text.match(/\bservice\s+\w+/g);
                if (matches) services += matches.length;
            } catch {
    ...
    private getSections(): SectionItem[] {
        return [
            new SectionItem('workspace', 'WORKSPACE', 'folder-opened',
                `${this.protoFileCount} files`),
