...
describe('DiagnosticsProvider Branch Coverage', () => {
  ...
  describe('shouldSkipDiagnostics', () => {
    it('should skip textproto files', () => {
  ...
  describe('missingSemicolon detection', () => {
    it('should detect missing semicolon on field', () => {
...
message Test {
  string name = 1 [(custom) = {
    ...
    it('should handle field continuation with number on next line', () => {
      const text = `syntax = "proto3";
message Test {
  string name =
    1;
    ...
    it('should handle next line starting with bracket', () => {
      const text = `syntax = "proto3";
message Test {
  string name = 1
    [deprecated = true];
...
message Test {
  optional group MyGroup = 1 {
    ...
    it('should error on groups in proto3', () => {
...
message Test {
  ...
  describe('oneof validation', () => {
    it('should check oneof naming conventions', () => {
      const text = `syntax = "proto3";
message Test {
...
message Response {}`;
    ...
    it('should check RPC naming conventions', () => {
      const text = `syntax = "proto3";
service MyService {
...
message Response {}`;
    ...
    it('should check for missing RPC request type', () => {
      const text = `syntax = "proto3";
service MyService {
...
message Response {}`;
    ...
    it('should check for missing RPC response type', () => {
      const text = `syntax = "proto3";
service MyService {
...
message Request {}`;
    ...
    it('should validate RPC types with workspace match', () => {
      // First add another file to workspace
      const otherText = `syntax = "proto3";
package other;
message OtherType {}`;
      const otherUri = 'file:///other.proto';
      const otherFile = parser.parse(otherText, otherUri);
      analyzer.updateFile(otherUri, otherFile);
...
service MyService {
  ...
  describe('option validation', () => {
    it('should validate boolean option with wrong type', () => {
      const text = `syntax = "proto3";
option deprecated = "true";`;
    ...
    it('should validate string option with wrong type', () => {
      const text = `syntax = "proto3";
option java_package = 123;`;
    ...
    it('should validate enum option with wrong value', () => {
      const text = `syntax = "proto3";
...
message Test {
  ...
  int32 name = 2;
    ...
    it('should detect duplicate map fields', () => {
      const text = `syntax = "proto3";
message Test {
  ...
  describe('reserved field validation', () => {
    it('should detect use of reserved field number', () => {
      const text = `syntax = "proto3";
message Test {
  reserved 1;
  string name = 1;
    ...
    it('should detect use of reserved field name', () => {
      const text = `syntax = "proto3";
message Test {
  reserved "old_name";
  string old_name = 1;
    ...
    it('should detect reserved map field', () => {
      const text = `syntax = "proto3";
message Test {
  reserved 1;
  map<string, string> data = 1;
  ...
  describe('field number range validation', () => {
    it('should detect field number out of valid range', () => {
      const text = `syntax = "proto3";
message Test {
  string name = 536870912;
    ...
    it('should detect field number in reserved system range', () => {
      const text = `syntax = "proto3";
message Test {
  string name = 19000;
}`;
    ...
    it('should detect map field number out of range', () => {
      const text = `syntax = "proto3";
message Test {
  ...
  describe('enum validation', () => {
    it('should validate enum naming conventions', () => {
      const text = `syntax = "proto3";
enum my_status {
  UNKNOWN = 0;
}`;
    ...
    it('should validate enum value naming conventions', () => {
      const text = `syntax = "proto3";
enum Status {
  unknown = 0;
}`;
    ...
    it('should validate first enum value should be 0', () => {
      const text = `syntax = "proto3";
enum Status {
  ACTIVE = 1;
}`;
    ...
    it('should detect duplicate enum values without allow_alias', () => {
      const text = `syntax = "proto3";
enum Status {
  UNKNOWN = 0;
  ACTIVE = 1;
  ...
  describe('nested message validation', () => {
    it('should validate nested messages recursively', () => {
      const text = `syntax = "proto3";
message Outer {
...
message Outer {
  ...
  describe('external dependency files', () => {
    it('should skip validation for .buf-deps directory', () => {
      const text = `syntax = "proto3";
message test_message {}`;
    ...
    it('should skip validation for vendor directory', () => {
      const text = `syntax = "proto3";
message test_message {}`;
  ...
  describe('field naming conventions', () => {
    it('should check field naming convention', () => {
      const text = `syntax = "proto3";
message Test {
  string UserName = 1;
}`;
    ...
    it('should check map field naming convention', () => {
      const text = `syntax = "proto3";
message Test {
  ...
  describe('message naming conventions', () => {
    it('should check message naming convention', () => {
      const text = `syntax = "proto3";
message test_message {
  string name = 1;
}`;
  ...
  describe('settings updates', () => {
    it('should respect disabled field tag checks', () => {
      provider.updateSettings({ fieldTagChecks: false });
      const text = `syntax = "proto3";
message Test {
  string name = 1;
  int32 id = 1;
    ...
    it('should respect disabled naming conventions', () => {
      provider.updateSettings({ namingConventions: false });
      const text = `syntax = "proto3";
message test_message {
  string UserName = 1;
}`;
