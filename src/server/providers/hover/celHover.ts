...
export function isCelContext(lineText: string): boolean {
  return (
    lineText.includes('buf.validate') ||
...
export const getCelHover: HoverHandler = (word: string, lineText: string) => {
  ...
  if (functionMatch) {
    ...
    const lines = [`**${matchedFnName}** *(CEL function)*`, '', `\`${fn.signature}\``, '', fn.description];
    if (fn.example) {
  ...
  if (inCelContext) {
    ...
    if (keywordMatch) {
      return createMarkdownHover([`**${matchedKeyword}** *(CEL keyword)*`, '', keywordMatch]);
    }
  }
